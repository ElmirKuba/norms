...
- [x] **9.28** — Фронт: `CryptoService` — ratchet-шаг: генерация новой ECDH-пары, передача нового `public_key` внутри зашифрованного сообщения (в заголовке, не в тексте), вычисление нового симметричного ключа через HKDF
- [x] **9.29** — Фронт: `chat_keys` расширить: хранить текущий + предыдущий ключ; 6 новых полей рачет-состояния
- [x] **9.30** — Фронт: `LocalChatRepository` — `updateChatKeyRatchet()`; `ChatEventsService` — `_decryptIncoming()` с трёхступенчатым fallback и `_advanceRatchet()` / `_commitRatchet()`
