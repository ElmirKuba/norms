...
- **Safety Numbers / верификация ключей чата** — актуально для публичных self-hosted инстансов где оператор потенциально враждебен своим пользователям. Для нашего инстанса и личных self-hosted — теоретический риск. Реализация: `fingerprint = SHA-256(pubA || pubB)` → первые 20 байт → 5 эмодзи из словаря 256 символов. UX: кнопка «Проверить безопасность чата» в деталях чата → модалка с 5 эмодзи → инструкция «сделай скрин и сравни с собеседником в другом мессенджере». Если эмодзи совпали — MITM не было.
