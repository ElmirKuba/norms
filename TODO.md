...
- [x] **9.24** — Фронт: `CryptoService` — Web Crypto API: ECDH X25519 генерация пары, HKDF-SHA256, AES-256-GCM encrypt/decrypt
- [x] **9.25** — Фронт: создание чата → генерировать ECDH пару, загружать публичный ключ, статус `pending_key`; UI pending_key в чатах
- [x] **9.26** — Фронт: WSS `chat_key_ready` → получить публичный ключ собеседника → AES-ключ → `chat_keys` → зашифровать и отправить все `pending_key` сообщения
- [x] **9.27** — Фронт: все исходящие шифруем AES-256-GCM перед WSS, входящие расшифровываем перед записью в `messages.content`
- [x] **9.27a** — Бэкенд+Фронт: peer-info (account_id, uin, nickname, username, system_name, device_nickname) в `chat_key_request` событии; responder сохраняет `peer_devices` сразу при получении запроса (а не ждёт следующего входа в чаты), JOIN в `getChats()` теперь возвращает peer на обеих сторонах
...
- [~] **9.28–9.30** — Первая попытка реализации (2026-05-14..16). **Отключена**: смешивала sending/receiving chains в один ключ → расшифровка падала после первого ratchet-шага. Подробный разбор бага и план корректной переделки: [`docs/double-ratchet.md`](docs/double-ratchet.md). Инфраструктура в SQLite/CryptoService сохранена как dormant.
- [ ] **9.31** — **Переписать Double Ratchet целиком** по плану в `docs/double-ratchet.md`: отдельные CKs/CKr, root key, HMAC chain ratchet, новый заголовок blob (flag=0x02), таблица `chat_skipped_keys`, out-of-order handling. **Acceptance:** 10 тест-кейсов из раздела 6 документа на трёх платформах.
- [ ] **9.32** — Тесты: симуляция компрометации ключа в середине переписки → убедиться что прошлые сообщения нельзя расшифровать, будущие — можно после ratchet-шага.
...
- [ ] **9.33** — Фронт: `CryptoService` — вычисление Safety Number: `SHA-256(pubA || pubB)` → первые 20 байт → 5 эмодзи (словарь 256 символов → каждый байт = один эмодзи)
- [ ] **9.34** — Фронт: кнопка «Проверить безопасность чата» в деталях чата → модалка с 5 эмодзи + инструкция «Сравни с собеседником голосом, при встрече или скриншотом в другом мессенджере»
- [ ] **9.35** — Фронт: QR-код в той же модалке — содержит fingerprint чата; кнопка «Сканировать QR собеседника» → если совпало → зелёная галочка «Чат проверен», флаг `is_verified` в локальной SQLite
- [ ] **9.36** — Фронт: в списке чатов и в шапке чата показывать иконку «проверен» (зелёный щит) если `is_verified = true`; рачет-ротации ключей внутри канала `is_verified` НЕ сбрасывают — канал уже доказанно чист, новые ключи путешествуют внутри него; сбрасывать только если чат пересоздан (новый первичный ECDH-обмен) или устройство собеседника сменилось
