...
### После MVP — «Небезопасные чаты» (Cloud чаты, опциональная фича)

_Реализовывать только после полного завершения шага 9 (включая Safety Numbers) и стабильного тестирования._

Идея: аккаунт-to-аккаунт чаты с хранением сообщений на сервере (без auto-delete при доставке). Сообщения всё равно шифруются перед отправкой и расшифровываются на устройстве — сервер видит только blob. Отличие от E2E-чатов (device-to-device): blob хранится вечно в БД и подгружается заново при каждом подключении. Доступно на всех устройствах аккаунта одновременно.

**Условия активации:**
- Обе стороны включили «Использовать небезопасные чаты» в настройках приложения (feature flag на уровне аккаунта, хранится в БД)
- При включении — предупреждение: «Сообщения будут храниться на сервере. Рекомендуется использовать обычные чаты (device-to-device).»
- При создании небезопасного чата — повторное предупреждение с рекомендацией выбрать конкретное устройство собеседника

**Создание чата:**
- Если у обоих включено — в `CreateChatModalComponent` появляется выбор: «Чат с устройством» (обычный E2E, рекомендован) или «Чат с аккаунтом» (небезопасный, с предупреждением)
- «Чат с аккаунтом» = cloud-чат, привязан к паре `account_id`, не к сессиям

**Хранение:**
- Отдельная таблица `cloud_messages` (без auto-delete, blob хранится бессрочно или по TTL из настроек)
- `cloud_chats` — пара `account_a_id` / `account_b_id` (аналог `chats` но account-level)
- При подключении — подгрузка истории с сервера (не только pending, а вся)

**Что продумать перед реализацией:**
- [ ] **N.1** — Проектирование: схема `cloud_chats` + `cloud_messages`, нейминг, связь с существующей архитектурой
- [ ] **N.2** — UX: как различать два типа чата в UI (иконка, метка); нейминг — «Небезопасный», «Облачный», «Cloud» или другой
- [ ] **N.3** — Правильное написание: «небезопасные» (слитно, прилагательное без противопоставления) или всё-таки другой термин в UI (например, «Облачные чаты» без негативной коннотации)
- [ ] **N.4** — Шифрование: единый ключ шифрования для cloud-чата (деривируется из чего? из аккаунт-ключа?), совместимость с мульти-девайсом (все устройства аккаунта должны расшифровывать)
- [ ] **N.5** — Синхронизация между своими устройствами: при входе с нового устройства — скачать историю с сервера, расшифровать
- [ ] **N.6** — Бэкенд: WSS-events для cloud-чатов (`cloud_message_new`, без auto-delete), `sendToAccount` вместо `sendToSession`
