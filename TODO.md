...
- [ ] **9.0** — Ревизия и переписывание `docs/` по готовому функционалу:
  - `api-contracts.md` — только реальные эндпоинты, реальные форматы, реальные коды ошибок
  - `auth-devices.md` — JWT-пара, сессии, кик, ротация — как реализовано
  - `backend-stack.md` — стек, модули, архитектура — как есть
  - `database-schema.md` — только реальные таблицы и поля (убрать всё что ещё не в БД)
  - `recovery.md` — Q&A флоу, rate-limit, reset_token — как реализовано
  - `identity.md` — UIN, username, логин — как есть
  - `invites.md` — инвайты, referrals, feature flags — как реализовано
  - `platform-services.md` — DI-сервисы, платформенные реализации — как есть
  - `frontend-architecture.md` — структура, конвенции — как есть
