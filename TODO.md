...
- **Safety Numbers / верификация ключей чата** ⚠️ до публичного релиза — Нормисы опен-сурс, любой может поднять свой сервер. Оператор враждебного сервера может подменить публичные ключи при обмене (MITM) и читать переписку пользователей. Без верификации пользователи об этом не узнают. Реализация: `fingerprint = SHA-256(pubA || pubB)` → первые 20 байт → 5 эмодзи из словаря 256 символов. Кнопка «Проверить безопасность чата» → оба видят одинаковые эмодзи если MITM не было → сверяют голосом/видео в приложении.
