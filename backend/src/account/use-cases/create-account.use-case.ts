...
export class CreateAccountUseCase {
  ...
  public async execute(dto: CreateAccountDto): Promise<CreateAccountResult> {
    ...
    if (!freeReg && inviteCode === null) {
      throw new BadRequestException(makeError(ErrorCode.INVITE_REQUIRED));
    }
      ...
      async (tx: DrizzleTransaction): Promise<TransactionResult> => {
        ...
        if (!freeReg && inviteCode !== null) {
          ...
          if (invite.expiresAt < new Date()) {
            throw new GoneException(makeError(ErrorCode.INVITE_EXPIRED));
          }
          ...
          if (deletedRows.length === 0) {
            throw new ConflictException(makeError(ErrorCode.INVITE_ALREADY_USED));
          }
