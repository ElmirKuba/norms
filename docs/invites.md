...
Флаги приходят с бэка по API (не захардкожены в бандл). Примеры:

| Флаг | Назначение |
|---|---|
| `devMode` | Показывать dev-элементы UI |
| `freeRegistration` | Регистрация без инвайт-кода (для разработки / раннего этапа) |

Конкретный эндпоинт и формат — при реализации бэка.
