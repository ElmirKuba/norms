...
- **Рабочая директория для git** — всегда `/Users/elmirkuba/coding/norms/` (корень репо). Все `git` команды запускать оттуда, независимо от того в какой поддиректории велась работа.
