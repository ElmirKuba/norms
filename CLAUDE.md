...
## Project Structure
- `application/` — Angular + Capacitor + Electron (свой `package.json`, один `ng build` → 3 платформы).
...
## Tech Stack
**Frontend (`application/`, один `package.json`, один `ng build` → 3 платформы):**
- Angular + TypeScript
...
## Important Decisions
- **Single `package.json` для фронта** (внутри `application/`): Angular + Capacitor + Electron. Платформенный DI-слой на рантайме определяет окружение. Scripts разделяют таргеты сборки. У бэка свой `package.json` в `backend/`.
